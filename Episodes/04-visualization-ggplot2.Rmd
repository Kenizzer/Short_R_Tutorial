...
```{r prep-data, eval=FALSE, purl=FALSE}
...
```{r boxplot-challenge-1, answer=TRUE, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = species_id, y = weight)) +
geom_jitter(alpha = 0.3, color = "tomato") +
geom_violin() 
```
...
```{r boxplot-challenge-log-scale, answer=TRUE, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = species_id, y = weight)) +
scale_y_log10() +
geom_jitter(alpha = 0.3, color = "tomato") +
geom_boxplot(outlier.shape = NA)
...
```{r boxplot-hindfoot-length-with-points, answer=TRUE, purl=FALSE}
...
```{r boxplot-challenge-2, eval=FALSE, purl=TRUE, echo=FALSE}
